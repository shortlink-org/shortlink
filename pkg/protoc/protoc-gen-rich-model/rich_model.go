...
func main() {
	...
	protogen.Options{}.Run(func(gen *protogen.Plugin) error {
		...
		for _, f := range gen.Files {
			...
			generateCommonFile(gen, f) // Generate common file content
...
func generateCommonFile(gen *protogen.Plugin, file *protogen.File) {
	filename := file.GeneratedFilenamePrefix + ".ddd.go"
...
func generateRichModel(gen *protogen.Plugin, file *protogen.File, message *protogen.Message) {
	filename := file.GeneratedFilenamePrefix + "_" + message.GoIdent.GoName + "_rich.go"
	g := gen.NewGeneratedFile(filename, file.GoImportPath)
	g.P("// Code generated by protoc-gen-rich-model. DO NOT EDIT.")
	g.P("package ", file.GoPackageName)
	g.P()
	...
	structName := message.GoIdent.GoName + "Rich"
	g.P("type ", structName, " struct {")
	g.P(message.GoIdent.GoName)
	for _, field := range message.Fields {
		fieldName := strings.Title(field.GoName)
