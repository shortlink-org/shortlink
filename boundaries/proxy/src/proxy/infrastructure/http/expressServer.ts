...
import express, { type Express, type RequestHandler, json, urlencoded } from "express";
...
const DEFAULT_URL_ENCODED_LIMIT = process.env.HTTP_URLENCODED_BODY_LIMIT ?? "1mb";

const BROTLI_ENABLED = typeof zlibConstants.BROTLI_PARAM_QUALITY === "number";

const configureApplicationDefaults = (app: Express, strictRouting?: boolean) => {
...
const registerMiddlewares = (app: Express, options: BuildExpressAppOptions) => {
  ...
  const compressionOptions = {
    ...
    // @ts-expect-error - brotli option is available at runtime on modern Node.js versions.
    ...(BROTLI_ENABLED && {
      brotli: {
        enabled: true,
        params: {
  ...
  app.use((req, res, next) => {
    const start = RequestContext.get<number>("request_start_time") ?? performance.now();
...
export const buildExpressApp = (options: BuildExpressAppOptions = {}): Express => {
  const app = express();
